...
<form method="POST" action="{{ $action }}" class="quote-form">
    ...
    <section class="panel mt-3">
        <div class="panel-header align-items-start">
            <div>
                <h2>Quote Items</h2>
                <p>Each line checks availability before the quote is saved or converted.</p>
            </div>
        </div>
    ...
    <script type="module">
                    ...
                    <div class="inline-edit-form quote-item-row" data-index="${index}">
                        <div class="row g-3 align-items-end">
                            ...
                            <div class="col-lg-1">
                                ...
                                <input name="items[${index}][quantity]" type="number" step="0.01" min="0.01" class="form-control quote-calc" value="${escapeHtml(item.quantity || 1)}" required>
                            </div>
                            <div class="col-lg-1">
        ...
        function calculateTotals() {
            ...
            $('.quote-item-row').each(function () {
                const row = $(this);
                subtotal += Number(row.find('[name$="[quantity]"]').val() || 0)
                    * Number(row.find('[name$="[no_of_duration]"]').val() || 0)
        ...
        $(function () {
            ...
            $('#quoteItems').on('change', '.quote-product', function () {
                ...
                if (Number(row.find('.quote-rate').val() || 0) === 0) {
                    row.find('.quote-rate').val(selected.data('rate') || 0);
                }
                calculateTotals();
            });
